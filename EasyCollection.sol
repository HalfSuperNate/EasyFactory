...
contract EasyCollection is EasyMintLogic {
    ...
    */
    ...
    constructor(address _newOwner) EasyInit(_newOwner){}

    // Initialize function to be called after deployment
    function initialize(string memory _name, string memory _symbol, address _newOwner) external {
        require(initialized == 0, "i");
        
        name = _name;
        symbol = _symbol;
        _transferOwnership(_newOwner);
        projectLeader = _newOwner;
        initialized++;
    }
    ...
    */
    ...
    /**
    @dev Allows admin to set the payout address and royalties for the contract.
    @param _address The new payout address to set.
    @param _royaltyPercentage The new royalty percentage less than or equal to 250 (25%).
    Note: address can be a wallet or a payment splitter contract
    */
    function setPayoutAddress(address _address, uint _royaltyPercentage) external onlyOwner{
        payments = payable(_address);
        if (_royaltyPercentage <= 250) {
    ...
    */
    ...
    function calculateRoyalty(uint256 _value) internal view returns (uint256) {
        if (_value > 0 && royaltyPercentage > 0) {
            return (_value * royaltyPercentage) / 1000;
        }
        return 0;
    }

    function transferRoyalty(uint256 _royaltyAmount) internal {
        if (_royaltyAmount > 0) {
            (bool success, ) = payable(payments).call{value: _royaltyAmount}("");
            require(success, "!R");
        }
    }
    ...
    */
    function _update(address from, address to, uint256[] memory ids, uint256[] memory amounts) internal virtual override {
        ...
        //Enforce Royalties
        uint256 royaltyAmount = calculateRoyalty(msg.value);
        if (royaltyAmount > 0) {
